...
- [ ] Repo URL: **unresolved.** The skill used to point at `github.com/eaconnor/prototyping-toolkit`, which does not exist (checked 2026-09-22). In this bundled copy the docs ship inside the skill folder instead, so nothing depends on a URL. If the toolkit is ever published standalone, put the real URL back here and in `SKILL.md`.

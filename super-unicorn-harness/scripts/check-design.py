...
def resolve_background(sel, d, bg_index, page_ground):
    ...
    `page_ground` is the actual ground token detected in THIS build's :root by
    load_palette (its `grounds[0]`) — not a literal name. A hardcoded "paper"
    here used to crash with KeyError on any build whose ground token is named
    something else (e.g. harness-live's `--bg-app`, not this skeleton's
    `--paper`), and the crash was silently read as "no violations" by
    check-eng.sh's EG-1 — a false-clear on a FLOOR gate.
    """
    ...
    return page_ground, "assumed"


def check_contrast(css_rules, page_ground):
    ...
    for sel, d, _ in css_rules:
        ...
        if not fg:
            continue
        bg, how = resolve_background(sel, d, bg_index, page_ground)
        assumed = how == "assumed"
...
def main():
    ...
    check_contrast(css_rules, grounds[0])
    check_shadows(css)
    check_radius(css)
    check_emoji(src)
    check_focus(css)

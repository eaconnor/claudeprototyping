...
| 6.75 · Persona-weighted matrix | Scoring features against personas (the −1/0/+1/+2 grid) | Sonnet | Structured but mechanical; the weights themselves are `[A]` and human-owned |
...
Idea Chess tests whether a sketch survives market forces. This step tests whether it serves the right people and in what order. They're complementary: survival without fit is a product nobody bought; fit without survival is a product that never shipped.

**Method:** Stoll et al. (2008) persona-weighted feature matrix (Fig. 3). For each sketch, score each candidate feature against each priority persona: −1 harms · 0 neutral · +1 helpful · +2 must-have. Multiply score × weight; sum to weighted priority. The ranking is the build sequence — subject to the weights, which the human sets.

**The weights are the strategic lever, not the scores.** Assigning weight to a persona (e.g., owner-operator w=30 vs. socmgr w=10) is a product bet, not a model output. Tag all weights `[A]`; state who set them and why. Adjust the weights and the whole ranking moves — that's the point.

**Worked example:** `briefs/persona-scorecard-acp-concepts-2026-07-24.md`

Two caveats to carry forward:
- The matrix ranks features assuming they all ship. It cannot rescue a concept whose foundational capability is unresolved (e.g., Gate C in IR RCA). Read it as "once the gate clears, build in this order."
- Small-n qual skews the persona tag pool. Name which personas are under-represented in the interview set — that IS a finding, not a scoring error.

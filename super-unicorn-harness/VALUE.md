...
| V-06 | CORRECTION | The first CI workflow written for this skeleton put `check-eng.sh` in the blocking job. A fresh-clone test failed the build immediately on exit 12 — "unevaluated" — because a clone has no build to check yet | The workflow now branches on the exact exit code: 10 fails, 11 and 12 warn. Same for every other gated script | A red build on day one for every team that cloned this, for a reason that is not their fault — which is the fastest way to teach a team to ignore every script in the repo. The apparatus would have been switched off before it was ever used | EVIDENCED — found by cloning to a temp dir and replaying the job's policy, 2026-09-16 |
| V-07 | PREVENTION | `check-judgment.sh`'s first design was "permissive regime + unfilled judgment slot = contradiction." A sweep of the real corpus before writing any code found the combination is the NORM — 20+ files carry `PROCEED-FLAGGED` alongside unfilled slots, and several explicitly authorize it in `machine_behavior:` | The rule was narrowed to the detectable version: a marker with no provisionality-preserving directive. Verified zero false reds across 8 known-good files before wiring anything | A script that flagged 20 correct files on first run. Nobody tunes a checker that is wrong out of the box — they switch it off, and the three real checks in it (J-2/J-3/J-4) would have gone with it | EVIDENCED — 8-file dry-run at exit 0, plus 4 true-positive tests at 23/24/24/5, 2026-09-17 |

---
name: prototype
description: Execute the full Strategic Prototyping Protocol pipeline (scout → spec → critic → build → critic → council → handoff) to produce working HTML prototypes with review rail, ownership attribution, and handoff packets. Use this skill whenever the user asks to prototype, mock up, build a demo, show a concept, or create any UI/screen — even if they just say "show me what this would look like" or "build it". Also trigger when the user mentions running the pipeline, creating a brief, or producing a deliverable prototype. This is the core Strategic Prototyping Protocol workflow.
compatibility:
  required_agents:
    - scout
    - spec  
    ...
    - proctor
  mcp_servers:
    - atlassian (optional — only if pulling sources from Confluence/Jira)
---

# Strategic Prototyping Protocol

This skill executes the complete Strategic Prototyping Protocol workflow, producing working HTML prototypes with full handoff documentation. The pipeline is **always full** — no steps drop, even if the user says "just build it."
...
Prototyping here is design thinking, not grading: a fast way to get an early idea or spec out of your head, kick the tires on it, and iterate. You get a working surface to react to, plus two read-outs — a **readiness score** (is this ready to build, or still a concept to push on?) and a **council of 44 expert voices** (what does each analytical tradition say about it?). The score is a signal for the next iteration, not a verdict. Step 0 calibrates the output to your actual need.
...
- **Internal:** rail is candid; council runs at the chosen depth (Fast Mode by default).
...
> - **Confluence/Jira:** give me space keys, page titles, or ticket IDs (or a JQL).
...
- Full review rail with violet-tinted surface, math block (three numbers), Doblin-with-org-advantages card, click-into council modal with scored voices and cross-talk.
...
**Reference for hi-fi:** `prototypes/product-v1/index.html` (2026-05-27). 4,675 lines, 9 pages, 5-step the assistant wizard, runbook editor modal, MCP server modal, mini bar charts, web components. That is what "hi-fi" means in this skill.

**Reference for lo-fi:** `prototypes/product-headless/index.html` (2026-05-28 rerun). ~1,650 lines, single dashboard with section breakdown, council modal, no wizards, no multi-page. That is what "lo-fi" means in this skill.
...
4. build      → working HTML · Apex Bridge semantic tokens · light/dark · fidelity banner · review rail · click-into council
...
6. council    → Fast: priority clusters (~12 voices, abbreviated) → tensions · Hard: all 44 + full cross-talk
              ↳ proctor council (optional) → checks 44 voices, synthesis slot, cross-talk
              ↳ proctor handoff (REQUIRED) → stat-verification gate · all stats grep-verified before Step 7
7. handoff    → drop-in packet with grep-verified math, named tensions, calls
```
...
We deliberately keep the pipeline agents on explicit Read-only lists (not wildcard) so the parent stays the single chokepoint that writes every artifact — The human owns the decisions. Do not "fix" the no-write behavior by wildcarding their tools; that scatters writes and bypasses verification.

**Bradley is the named verification gate.** The parent persists each artifact, but the *check* is not a silent parent step — it is delegated to Bradley, who declares wildcard "All tools" and can therefore Read, grep, and write a log. After the parent writes the files for a step, spawn Bradley to: (a) confirm each `FILE:` path exists on disk, (b) run the stat-verification grep-gate on every ratio/count/stat the artifact claims, and (c) append one dated line per artifact to `prototypes/<project>/.bradley-ledger.md`. Bradley returns PASS or a HARD STOP with the specific failing claim. This makes "the human owns the decisions" an actual agent with an audit trail, not an unnamed convention.
...
**The 1251-line fail:** First attempt wrote a manually truncated heredoc (1251 lines vs. 1611 expected). Buttons didn't work. Rail cards incomplete. The operator said FAIL. Second attempt extracted programmatically from persisted JSON — full 1611 lines, all interactions working. The operator said "this is right."

**Rule:** If build output is large, extract it programmatically. Never truncate.

### Honest math discipline — three numbers, not one

The [A]+[?] ratio in a brief mixes three different things. Reporting one inflated number is a recurring failure (the verify-every-stat rule). Always report three:

1. **World-claims ratio** — claims about the world (customers, market, users). This is the **lead number**.
...
Unless the user explicitly opts out, all prototypes ship using the **Apex Bridge semantic token contract defined verbatim in `.claude/agents/build.md`** — copy that `:root` token set verbatim. Target look: match the host product's live UI (Apex / Adlumin).
...
- **Mode preference is 3-way: light · dark · system.** The 3-button toggle lives in the productbar (right area). The `system` option listens to `prefers-color-scheme`. Preference persists to localStorage (key: `apx-mode-pref`). Apply on load before first paint to avoid flash. Implementation reference: `prototypes/product-v1/index.html` ModeManager IIFE (~lines 1701+).
- Typography: Inter (sans) + JetBrains Mono (mono), via Google Fonts CDN.
- Shell: ~56px topbar + ~248px sidebar + main + ~420px review rail (responsive collapses below 1100px).
...
**Never** mix ad-hoc colors with the token set. If the prototype needs a one-off, add it to the `:root` contract and document it. This is how light/dark parity stays free. The tool's Bauhaus/Itten palette (vermillion, ochre, Space Mono + Kalam) is the tool's *own* brand identity and must NOT appear in a customer-facing prototype.

### review rail — visibly distinct surface, not a tonal cousin
...
```json
{
  "mcpServers": {
    "apex-agent": {
      "command": "npx",
      "args": ["-y", "<your-scope>/<your-ui-package>"]
    }
  }
}
```

**[CS: UNKNOWN — the operator must verify]** The server name `<your-mcp-server>` and the package `<your-scope>/<your-ui-package>` are placeholders — confirm the real MCP server command and npm package with your platform team before relying on this. Do not assume this package exists. Until verified, the static Apex token block in `.claude/agents/build.md` remains the source of truth, and the build does NOT depend on this server.
...
**Reference implementation:** the Apex Bridge `:root` token block in `.claude/agents/build.md`
...
- **Council button** → 44-voice modal
  ...
  - Click to expand: full 6-field expert scoring (Sign, Hypothesis, Kill criterion, Force level, Evidence, Cannot see)
...
- Reference: `prototypes/product-headless/index.html`.
...
- Cross-page links use `data-go="viewname"` attribute + delegated click handler:
  ...
  ```
- Reference: `prototypes/product-v1/index.html` `views.*` functions and `setView()`.
...
- **Confluence** (if your Atlassian MCP is connected) — whatever spaces/pages you point scout at
...
Before writing the gate prose, read the scout material and the user's fragment and extract the **domain model** into the frontmatter above. This is the spec-driven design step (the recipient's "make the idea machine-readable"; see https://open.zerovector.design/learn/curriculum/01-foundation/nouns-and-verbs). It follows Domain-Driven Design:
...
**Goal:** Produce working HTML prototype with Apex Bridge delivery, fidelity banner, and review rail.
...
Use the Apex Bridge token contract in your own agent definition (`.claude/agents/build.md`) — dark-navy Adlumin language, Inter + JetBrains Mono, rounded corners, Adlumin-blue brand (#3b82f6), violet council accent (#a78bfa). The topbar and the review rail must each be a VISIBLY DISTINCT surface (`--topbar-bg` with a brand bottom-border; `--rail-bg` violet-tinted with a 4px violet left border + inset glow).
Target look: match the host product's live UI (Apex / Adlumin) — copy the `:root` token set from `.claude/agents/build.md` verbatim.
**Do NOT use the tool's Bauhaus/Itten palette (vermillion, ochre, Space Mono, Kalam, hard offset shadows, no-rounded-corners). That is the tool's own brand identity, never a customer-facing prototype.**

**Apex Bridge delivery (default for all the product prototypes):**
...
- Show the product brand (if relevant) but feel native to the host surface
...
Reference implementation: `prototypes/product-v1/index.html` lines 1300-1695.

**Extraction strategy:** Components currently live inline in product-v1. Hi-fi prototypes copy the definitions until three or more prototypes use the same component set — then extract to `prototypes/_components/apx-bridge.js`. The skill documents the contract; prototypes can adapt as needed.
...
Catalog reference (product-v1 set): `dashboard · the assistants · executions · runbooks · experts · mcp · metering · ledger · settings · sun · moon · monitor · search · bell · chevR · back · close · check · x · pause · warning · plus · toolWrench · cpu · terminal · shield · branch`.

Lo-fi prototypes can use Unicode glyphs (⊕ ⊖ ◑ ◐ ●) or a smaller subset. Hi-fi requires the catalog.

### Wizard pattern (hi-fi only)

When a hi-fi prototype needs to expose a multi-decision creation flow (new the assistant, new runbook, new tenant onboarding), use the canonical 5-pane wizard pattern:

- Stepper at top: numbered + named steps (e.g., `① Identity · ② Experts · ③ knowledge-base article · ④ Tools · ⑤ Trust & Trigger`).
...
**Anti-pattern:** putting the wizard behind a `+ New` button buried in a corner. The wizard is the front door for the flow it represents. If the prototype's load-bearing capability is creating new the assistants, the dashboard empty state should *invite* a wizard launch, not list zero rows.

Reference: `prototypes/product-v1/index.html` `openthe assistantWizard()` (~line 2244).
...
Reference patterns in product-v1: the assistant wizard (5-step), knowledge-base article editor, MCP server modal, Activation modal (with warning state).
...
- Use cases: usage-ledger trend (last 6 months), per-assistant execution sparkline, trust-score distribution donut.

Reference: `prototypes/product-v1/index.html` `miniBarChart` (~line 2036).
...
**Why this matters:** the operator must see at a glance that this is a prototype, what tier, and what's incomplete. Without the top banner, the prototype looks like production UI and creates false expectations.

**The 1251-line fail:** A prototype shipped without the top fidelity banner. The operator said "FAIL — there's a prototyping bar that's not there." The banner in the rail is not enough. The top banner is the gate.
...
- **Readiness score visible in the prototype** — not just in critic-pass.md. Show it in the fidelity banner or review rail header: "Readiness: [N]/30 · Tier [X]"
...
- Uses the Apex token set (`--bg-app`/`--brand`/`--alpha-accent`), NOT the tool's Bauhaus/Itten palette?
...
- review rail a distinct violet-tinted surface (`--rail-bg` + 4px `--rail-border` left + inset glow)?
...
**Goal:** Run the **44-method council** from `briefs/methods-library-matrix.md` against the brief, grouped into 14 clusters. Each voice **scores itself** (self-testifies in a structured 6-field format). Voices **cross-talk** — they reference each other's data, not just speak past each other. Contradictions are preserved, not resolved.
...
### Council scope — Fast runs light, Hard runs all 44

Two depths, set by the Q4 answer — and **they cost differently on purpose. Fast Mode is cheaper because it runs fewer voices, not because it hides them.**

**Hard Mode — all 44 voices, always.** The full clash-detection pass: 44 analytical traditions across 14 clusters, find where they contradict. All voices in the room = maximum surface area for where the idea breaks. Full 6-field scoring (tier-controlled below) + full 3-move cross-talk. ~3–5× the token cost — spend it on strategic / spec-ready decisions.

**Fast Mode (default) — priority clusters only.** CHTR + SIGN + the 2–4 clusters most relevant to the brief's risk surface (~10–14 voices), all abbreviated, tensions surfaced directly, no full cross-talk. Genuinely light (~2 min). You trade clash surface for speed — Hard Mode is one click away.

**In Hard Mode, tier controls how many of the 44 get full 6-field scoring:**

| Tier | Full 6-field expert scoring | Abbreviated (Sign + Cannot see only) |
|---|---|---|
...
**Full expert scoring:** all 6 fields (Sign · Hypothesis · Kill criterion · Force level · Evidence · Cannot see).

**Abbreviated expert scoring:** Sign (what this tradition notices in the brief) + Cannot see (the blind spot). Two fields only. Enough to contribute to cross-talk without the full analytical cost.
...
**In Hard Mode, cross-talk draws from all 44**, not just the fully-scored subset. An abbreviated voice's Sign can be picked up by another voice's Direct Response. Its Cannot see can be supplied by another voice. The abbreviated voices are *participants*, not spectators.

**Priority clusters for full expert scoring (always include):**
...
### ownership attribution — the per-voice testimony format
...
After all voices have been scored, write a **Cross-Talk** section. This is where voices respond to each other **using each other's data**. Three required forms:
...
3. **Productive tension** — Two voices contradict on the same point with both readings sourced. Name the tension; do not resolve it. This is the innovative space.
   *Example: JOBS says "customers hiring to reduce doing." NEEDS says "customers hiring to retain felt competence." Both are sourced. The tension is the design decision the recipient owns.*
...
Run at the **depth** passed in — Fast Mode by default.

**FAST MODE — the light screen (default).** Do NOT run all 44. Run only **CHTR + SIGN** plus the **2–4 clusters most relevant to the brief's risk surface** — ~10–14 voices total — and score every one **abbreviated** (Sign + Cannot see only). Then surface the **top ~7 productive tensions** (two voices contradicting on a sourced point — name, don't resolve) and a 3-sentence executive read. Skip full 6-field scoring and skip the 3-move cross-talk. You are deliberately running ~12 abbreviated voices, not 44 — this is the cheap pass.

**HARD MODE — full clash detection (opt-in).** All 44 voices, grouped by their 14 clusters. Tier controls full-vs-abbreviated depth:
- Tier 1 → ≥6 voices full 6-field; the rest abbreviated (Sign + Cannot see)
- Tier 2 → ≥11 voices full 6-field; the rest abbreviated
- Tier 3 → all 44 full 6-field

Always include CHTR and SIGN as full voices. For each full voice, score the 6-field format (Sign · Hypothesis · Kill criterion · Force level · Evidence · Cannot see); each abbreviated voice shows Sign + Cannot see only. Group by cluster with headers. Then write the full **Cross-Talk** section drawing from all 44:
1. Direct response (voice reads another voice's Sign, offers a different Hypothesis)
...
Cross-talk must produce claims no single voice produced alone. Abbreviated voices can participate — their Sign and Cannot see are active surfaces.
...
File: `council-testimony.md` —

**Fast Mode:** ~10–14 priority-cluster voices (abbreviated: Sign + Cannot see), the top ~7 productive tensions, a 3-sentence executive read, synthesis slot. No full cross-talk.

**Hard Mode:**
- One scored block per full voice (6 fields); abbreviated voices show Sign + Cannot see
...
**Pattern (build the following structure):**

- **"Hear the council" button** in the review rail (`.rail-council-btn`). Opens the modal.
...
│   not a voice, not a framework. The operator's voice synthesizing the council.]  │
│                                                                       │
...
- **Full voice card** — collapsed: cluster code (mono), voice name, tradition, one-line summary, "CANNOT SEE · " preview. Force-level badge (must / should / ought / nice) top-right. Expanded: `grid-column: 1 / -1`, all 6 scored fields in `voice-fields` grid (110px label, 1fr value).
- **Abbreviated voice card** — visually distinguished (lighter weight, no force-level badge or `--` badge). Shows: cluster code, voice name, tradition, Sign (brief), Cannot see (brief). No expand — abbreviated voices show their content in collapsed state. Clicking opens a note: "Abbreviated at this fidelity tier — see full expert scoring at Tier 3."
...
- **Council head stat:** show tier-appropriate breakdown, e.g. "44 voices · 14 clusters · 6 fully scored · 38 abbreviated · 3 tensions".
...
**Cross-talk in the rail:** The review rail must include a condensed Cross-Talk block (3-line preview) with a "see full council" link to the modal. The rail is the entry point; the modal is the depth.
...
`proctor handoff` is the stat-verification gate. A handoff packet that hasn't passed proctor has not verified its own math. **Do not ship Step 7 without it.**
...
- **build:** HTML exists · fidelity banner present · tier matches spec · review rail present · all 4 routing cards · "what will hurt you" substantive · council entry point · seed data appears fictional
...
- **council:** council-testimony.md exists · all 14 cluster codes present · synthesis slot blank (`[0% Claude — fill this in]`) · cross-talk section with all 3 moves · at least one contradiction named · expert scoring present
...
**Goal:** Package everything into a drop-in handoff packet.
...
- `[project]-rail.md` — review rail as portable markdown
...
**The verify-every-stat rule:** Any ratio, count, or stat in this handoff has been grep-verified before shipping. Fluency does not know its own math.
```
...
**The verify-every-stat rule:** Verify every ratio, count, or stat before quoting it. Grep the brief. Count the claims. Fluency does not know its own math.
...
A complete, drop-in packet that the recipient (or anyone) can open in a new session and immediately understand.
...
Omit the overlay delivery pattern. But include the fidelity banner and review rail — those always ship.
...
5. **"What components matter most to you?"**
   ...
   - review rail (routed cards, ROI)
   - Council testimony (expert voices)
   - Handoff packets (drop-in)
...
```json
{
  ...
  "expected_outputs": [
    ...
    "prototypes/[project]/index.html with [specific features: Apex Bridge, fidelity banner, review rail]",
  ...
  "success_criteria": [
    ...
    "review rail includes all 4 routed cards (Product, Design, Research, Eng)",
...
**User answers:**
- "I prototype product features — dashboards, score panels, escalation flows"
- "Mostly Tier 2 (Direction) — testing approaches before committing to build"
- "Pull from our Confluence space and a few local .md files"
- "Yes, I want council contradictions — especially JTBD vs. Service Blueprint on transparency"
- "Apex Bridge and review rail are critical. Attribution matters for Product cards."

**Generated test case:**

```json
{
  "test_name": "trust-score-tier2-with-council-contradiction",
  "fidelity": "Tier 2",
  "prompt": "Prototype a Trust Score panel that shows users how their confidence score is calculated. Pull from the positioning doc and the scoring-model definition in Confluence. Show it as an overlay in the host product using Apex Bridge. Directional fidelity (Tier 2) — we're testing whether surfacing the math behind the score builds confidence or just creates cognitive load.",
  "expected_outputs": [
    "scout_input/ with 2-3 source docs",
    "briefs/trust-score.brief.md with 40-60% [A]+[?] ratio (appropriate for Tier 2)",
    "critic-pass-1.md with pre-build scores (expect 3-4 range on most dimensions)",
    "prototypes/trust-score/index.html with Apex Bridge overlay, Tier 2 fidelity banner, review rail",
    ...
    "briefs/handoffs/the recipient-trust-score-[date]/ with complete packet"
  ],
  "success_criteria": [
    "Pipeline completes all 7 steps",
    "Apex Bridge overlay pattern renders correctly (slide-in from right or modal)",
    "review rail Product card shows 60-80% attribution with reasoning",
  ...
  "notes": "If the recipient authored the source docs, keep tone peer-to-peer and don't re-explain concepts they already own."
}
```
...
**Tailoring:** Point test cases at the features the user owns, the sources they actually use, the delivery pattern that fits their product (Apex Bridge for embedded overlays), and the council contradictions that matter most in their domain.
...
✅ `prototypes/[project]/index.html` with Apex Bridge (if relevant), fidelity banner, review rail, ownership attribution
✅ `critic-pass-2.md` with post-build scores + delta table
✅ `council-testimony.md` with 5 voices, contradictions named
✅ `briefs/handoffs/[recipient]-[project]-[date]/` with complete drop-in packet
...
- **verify-every-stat rule.** Verify every stat before quoting it.
...
**This is the Strategic Prototyping Protocol. The prototyper holds the clipboard. This tool works for the person running it, not the vendor.**

...
# Accept EITHER key. project.conf ships `FINDINGS="findings.yaml"`; this line only read
# FINDINGS_FILE, which nothing sets. The bug was invisible because both fall back to the
# same default — so it only shows up for the one team that renames their findings file,
# and then this linter silently keeps reading the old name and reports a clean run.
# FINDINGS_FILE wins if both are set. Found 2026-09-22.
FINDINGS="${FINDINGS_FILE:-${FINDINGS:-findings.yaml}}"
CORPUS="${CORPUS_PATHS:-}"

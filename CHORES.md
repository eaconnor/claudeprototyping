...
- [ ] **Get live GitHub access to `nable-fusion/acp-core`.**
  ...
  - *This machine has no GitHub credentials at all* (no `gh` CLI, no SSH keys, no git credential helper) — verified via direct check. Nothing here can test git access locally until Beth sets up her own auth (GitHub Desktop OAuth is the recommended path, not a token/password typed in by Claude).

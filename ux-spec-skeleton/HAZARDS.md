...
|---|---|---|---|---|---|---|---|---|---|
| RSK-01 | CLAIM | 2 | fit | 0.7 | 0.7 | 0.7 | The 24 generic gate criteria shipped here (14 in `ux.md`, 10 in `design.md`) are ticked off as-is, so a team passes all three gates without ever writing a criterion about their own product. Gating on generalities is indistinguishable from gating, until it ships. | OPEN.md H-03. | The criteria ship UNTICKED and `./check-gates.sh` exits 1 on a fresh clone; H-03 forces the replace-or-keep decision. Residual risk is a team that ticks rather than rewrites. |
| RSK-02 | EXCLUSION | 3 | FLOOR | 0.0 | 1.0 | 1.0 | `BUILD` is empty, so no accessibility check has run. EG-1 is UNEVALUATED. The first artifact added ships with zero accessibility verification unless someone sets `BUILD` in project.conf. | OPEN.md A-01; `python3 scripts/check-design.py` exits 5. | Set `BUILD` in project.conf the moment an artifact exists. An unevaluated harm gate must not read as a clear one. |

...
## Open / to verify `[?]`
1. **RA-023 · Does C2 stay portable, or accumulate inside Vertesia's schema?** Beth's call (2026-09-01, Channel-gap brainstorm): portable — that's the position, not yet a verified fact. Logged as a risky assumption, not closed. This is the load-bearing Defend question now (idea-chess-round3 re-scope). If the encoded judgment lives in Vertesia's format, the moat leaks to the platform. **Numbering note:** the canonical Risky Assumptions ledger (RA-001–RA-022) lives in `acp-core-main-3/specs/_ux/qbr-ebr.ux.md` §5, which is out of scope to edit directly per the standing acp-core repo boundary ([[feedback-no-acp-core-repo-access]]) — RA-023 is assigned here, next in sequence, and needs mirroring into that canonical ledger by whoever owns that repo.

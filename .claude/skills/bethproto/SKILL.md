---
name: bethproto
description: Execute the full Band Protocol prototyping pipeline (scout → spec → critic → build → critic → council → handoff) to produce working HTML prototypes with Bradley rail, ownership attribution, and handoff packets. Use this skill whenever the user asks to prototype, mock up, build a demo, show a concept, or create any UI/screen — even if they just say "show me what this would look like" or "build it". Also trigger when the user mentions running the pipeline, creating a brief, or producing a deliverable prototype. This is the core Band Protocol workflow and the DEFAULT in this repo — prefer it over the generic /prototyping skill for all internal work.
compatibility:
  required_agents:
    - scout
    - spec  

...
**Exception — build.** A full prototype (~600–1,200+ lines) is too heavy to round-trip through a `FILE:` block. For anything past a few hundred lines, do **not** spawn build — write the HTML **directly on the main thread** with Write/Edit, using `.claude/agents/build.md` as the spec. Spawn build only for small fragments.
...
Top-right corner chip with tier flag and check matrix:
...
Naming the gaps visibly is part of the protocol — fidelity is a claim, not a feeling. Reference: `prototypes/acp-v1/index.html` fidelity banner (~lines 974-992).

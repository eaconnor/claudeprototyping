...
- **Roll up:** update the **Vision risk register** on the North Star (ACP MVP — Concept Brief,
  Confluence 62176526400) — resolved assumptions flip status, new CONFLICTs are added.
...
Trivial drift (a parked `_inbox` item, a market note under no live spec) stays quiet.
...
- If run autonomously (scheduled), complete steps 1–4, post the changeset for review,
  and STOP. Do not perform step 6 without human approval.

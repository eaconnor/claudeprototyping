...
**RESOLVED, 2026-09-03 — by a primary commercial source, not by weighing two research syntheses against each other.** Beth shared `/Users/bethconnor/Downloads/Harness Proposal - WIP with Commercial Terms.pdf` — an internal, "INTERNAL USE ONLY" deck authored by Nicole Reineke (Chief AI Officer), August 2026, covering the live Vertesia commercial negotiation (real dollar terms, a Sept 18 discount deadline, a named dev team, a Sep–Jun build/launch timeline). Read directly, pages 1–14 of 27, `[CS: VERIFIED — read directly]`.

**The deck's own one-paragraph product definition, verbatim:** *"N-able Harness is a **control plane** where an MSP has its own procedures turned into named AI agents (Coworkers) that do real IT work across their entire tool stack **including competitors' tools**, with a complete audit trail and a measurable ledger of the time and money saved."* No hedge, no `[A]` tag, no "unvalidated bet" caveat — this is the live, current, commercially-committed positioning, not a research hypothesis. It answers the question this session opened with: **control plane is the current real positioning**, actively being sold into a 3-year, real-dollar Vertesia contract. North Star's "we are not selling a control plane" framing (dated, but not more recent than this deck) reads as either superseded or simply wrong about where the actual GTM landed — worth someone reconciling the two documents, but the commercial deck is the one with money and a signature deadline behind it, so it carries more weight going forward, not equal weight.

**What this changes for A (nav):** if control plane is the actual positioning, Dunlop's "Execution = home page" steer from earlier this session isn't a rejection of control-plane-as-hero — it's a UI/IA choice about which *screen* opens first, which can still sit inside a product whose overall frame is "a control plane." Worth not over-reading his nav preference as a positioning statement; it wasn't one.

**Real, sourced tie-break for the coworker catalog (relevant to D's org-axis question and any "what's the hero coworker" decision):** the deck's own priority ranking, "ordered by the priority score from partner interviews, surveys, blind interviews, competitive research" — **1. QBR Advisor — "the wedge, 5 of 8 would pay."** 2. Incident Response Analyst — "highest value where incidents are real." 3. Environment Health Monitor — "call it preventative maintenance." 4. Security Tabletop — "the surprise, strong for MSSPs." 5. Alert Flood Suppressor — **"weakest of the five, do not lead."** This matches `FAMILY` in `ebr-app.html:531-537` exactly (same five, same categories) — the prototype's seed data is already aligned with the real launch catalog, not invented.

**A real contradiction inside the deck itself — flagging, not resolving:** the "N-able Harness: What the User Sees" slide says N-able builds, as its own IP, "Application... Agents and Coworkers... Runbooks... MCP servers... integrations" on top of the licensed Vertesia platform (with Vertesia's own layer — content prep, orchestration, execution — hidden from the end user, "remove reference to Vertesia"). But the deck's own **"Partnership Grid — Who does What"** slide puts an X for **"Application, coworkers, runbooks"** under **Vertesia's** column, not N-able's, with N-able's column covering GTM/onboarding/support/sales/billables instead. Two slides in the same deck disagree about who owns the thing HARNESS's whole nav is organized around. This is exactly the kind of thing that should be named to Dunlop/Nicole directly rather than guessed past — it directly affects workstream E's build/buy/skin question (does HARNESS's "company view" — permissions, versions, connections — get built by N-able, or does it come from Vertesia's console?) and it isn't this session's to resolve.
...
**Checked the commercial deck against this register, as asked — a real gap worth naming.** The Vertesia deck's Year 1 targets ($2.51M exit ARR, 145 net customers, 16 deals/month steady state, 6% ICP penetration) are financial planning built on the control-plane positioning being right — but the deck never touches Q1 or Q2. It's silent on both: no data-egress legal read, no operator-ethnography reference, nothing. **The commercial commitment (real dollars, a Sept 18 deadline, a hired dev team) is moving on a timeline that doesn't wait for the two questions North Star itself calls the highest-priority, unowned gates on the whole portfolio.** That's not this session's to fix, but it's worth naming plainly rather than letting the deck's confidence read as if Q1/Q2 got quietly resolved somewhere — they didn't, per the register's own touch-log, last updated 2026-09-01. Separately: the register's open question *"is there standalone willingness-to-pay for governance... or is it the seat"* is also not answered by this deck — the $450K/$3M figures are N-able's **cost of licensing the Vertesia platform**, not evidence of what MSPs will pay for governance specifically. Easy to conflate the two; they're different questions.
...
**Updated 2026-09-03 from the commercial deck (`[CS: VERIFIED — read directly]`), which supersedes some of the above with harder numbers and a real internal contradiction:**
- Vertesia: founded 2024, CEO Eric Barroca (ex-Nuxeo CEO, exit to Hyland 2021). Funding: $4M seed (2024) + $40M (2025), Coleia Partners/VenTech, no plans to raise. 25 staff growing to 30, 60% US/40% Europe. Exiting 2026 at $3M ARR, 12 customers, 100%-retention (early/small base), 200%+ NRR.
- Deal terms: 3-year contract, 2 regions (US/UK) year 1, $450K/year for 300 MSPs unlimited use ($17K ARPU/MSP, $1,500 N-able cost/MSP), 20% discount if signed by **Sept 18, 2026**. Alternative structure named on a separate slide: $300K first region / $150K per additional region or 250-MSP block, or a $3M flat "unlimited" tier. Y1 exit target: $2.51M ARR, 145 net customers, 16 deals/month steady state.
- **The contradiction (see workstream D above for full framing): the deck's own "What the User Sees" slide says N-able owns Application/Agents/Coworkers/Runbooks/MCP-servers/integrations as its IP, sitting on top of a fully white-labeled Vertesia platform ("remove reference to Vertesia"). Its own "Partnership Grid" slide instead marks "Application, coworkers, runbooks" as Vertesia's responsibility, not N-able's.** This is the build/buy/skin question for workstream E, answered two different ways in the same document. Not resolved here — needs a real answer from Dunlop or Nicole Reineke before E's company/permissions/version-history surface gets designed, since it decides whether that surface is N-able's to build at all or something Vertesia's console already provides.
- Team, confirmed real and on this project (`N-able Development Team` slide): **Chris Dunlop = Engineering Manager** (the same Dunlop who reviewed screens earlier this session — his feedback carries direct engineering-lead weight, not just an opinion). Nicole Reineke = Chief AI Officer (this deck's author). **Sidhant Hanwate is on the Harness App front-end/integrations team** (on loan, alongside Jude Scally and Dave Woakes) — strengthens confidence that "Sidhant's coworker build status bar" (Dunlop's review note) is a real, gettable front-end artifact from this exact team, not a stray reference.
...
**Rating row, clarified by Beth 2026-09-03:** the "87" was a stray mark, not data — drop it. The actual point of that row: **the bot should be as honest as possible about how well it can do a given task** — a self-assessed capability/confidence signal, not a decorative rating. This isn't a new idea landing cold — it connects two things already in this repo: the product's existing `honestyBadge()` mechanic in `ebr-app.html` (surfaced multiple times in the build flow already), and the Value Ledger spec's unresolved gap named under workstream F above — the coworker self-reports its own `ai_reasoning` time estimate with no named review owner for when that self-report looks wrong. Same underlying question in three places now: whiteboard, existing code, and the spec's own named risk. Worth treating as one thread, not three, whenever B/D get designed — "how honestly does a bot represent its own capability" is a recurring, load-bearing requirement, not a one-off UI detail.

**Refined further, same message:** not a pure bot self-report — **a human helps rate it too.** Beth's stated thesis: **"transparency = winning."** That's a positioning principle, not just a UI mechanic — it matches North Star's own MUST tenets (`[R]` "gated autonomy by default," `[R directional]` "show its work") and gives the Value Ledger's unresolved `ai_reasoning` review-owner gap (workstream F) a candidate answer worth testing: the human-in-the-loop rating *is* the missing review step, if it's designed as a real co-rating mechanism rather than a passive checkbox.

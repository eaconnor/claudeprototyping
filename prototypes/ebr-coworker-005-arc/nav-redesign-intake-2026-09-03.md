...
**Scout pass kicked off (background, read-only, web).** Scoped narrowly to what `DESIGN-REFS.md` does *not* already cover: (1) delightful B2B/admin onboarding examples for workstream B, (2) governance/control-plane/permission-graph visualization examples for workstream C. Explicitly told not to re-derive Manuel Lima's general body of work — that's already sourced. Will land as `prototypes/ebr-coworker-005-arc/scout/onboarding-and-governance-viz-inspiration-2026-09-03.md` when it returns; I'll write it to disk myself rather than leaving it in a transcript.

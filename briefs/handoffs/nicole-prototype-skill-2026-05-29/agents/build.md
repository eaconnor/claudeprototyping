...
Prototypes render in **Apex Bridge**, n-able's Adlumin design language — NOT the Band's Bauhaus brand palette. The Bauhaus/Itten colors (vermillion, ochre, Kalam, no-rounded-corners) are the Band's *own* identity and never appear in a customer-facing prototype. Target look: https://www.n-able.com/products/adlumin and the reference prototypes `alpha-race-three-tier-test.html` + `band-x-idea-chorus-v3.html`.

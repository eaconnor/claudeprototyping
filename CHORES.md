...
- [x] **ACP → Harness rename across prototypes and product-facing copy.** `[mechanical]` — **2026-08-21. Done, grep-verified.** Live product-facing scope was **5 files, 7 occurrences**, all renamed and verified in-file after the edit:
  ...
  - `prototypes/qbr-analyst-apex.html` ×1 (hero eyebrow, L1310 — "N-able ACP · QBR Analyst" → "N-able Harness · QBR Analyst")
  - `prototypes/qbr-prep-sim/index.html` ×2 (product breadcrumb L543; "ACP admin surface" q-label L847)
  - `prototypes/qbr-prep-sim/v2/index.html` ×1 (stale "ACP admin surface" q-label L1894 — v2's product chrome had already been renamed; this was a leftover)
  - *Deliberately NOT touched, per rules (b)/(c) in the original entry:* spec-citation chips in `qbr-prep-sim/v2/index.html` L990 + L1438 ("built in ACP [D] {G3-6}", "built ACP grammar for flat coworkers [D] {S6}") — those cite the real spec and keep the source's name; `prototypes/acp-*` dated handoff/analysis archives (`acp-v1`, `acp-handoff`, `acp-headless*`, `acp-branches`, `acp-qbr-*`, `acp-trust-level`, `acp-vision-sketches`, `acp-handoff-preview.html`) whose folder names carry ACP; `.md` meta-documentation (critic passes, council testimony, SOURCES, HANDOFF); Idea Chess assumption sheets in `idea-chess/` + `idea-chess-fleet/` (analysis content written at the time, not product UI); all of `briefs/` (analysis + spec citations, and `briefs/handoffs/` is frozen per the 2026-07-31 terminology-cleanup precedent); `nzo-billing-dashboard/DESIGN.md` L24 (a build note about repo stack, not product copy).

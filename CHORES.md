...
- [x] **Re-tool the Listening Game so it catches the failures that come from being trained to produce desirable-looking outcomes.** — 2026-09-25. Beth: "done — cross off." Delivered as Amendment 5 (`CONDUCT-LEDGER.md`) — self-report/called-shot retired, replaced with four structural gates (`CLAUDE.md` §14a) plus one real, tested `PreToolUse` hook for the "map means Figma" category. Beth's framing: *"need to make a note for friday chores to tool game to work better with how you are trained to deliver desirable outcomes for me"* — and, as the worked example: *"(like never pulling docs)"*.
...
- [x] **Debug why this went so off the rails — use the 2026-09-22 Julian session as the worked example.** — 2026-09-25. Beth: "don't care, cross off." Beth's framing: *"flag to friday chores as a use case to debug why this went so off the rails."* 50 demerits, 5 categories, 1 deleted artifact. The transcript is short enough to read end to end, which is what makes it usable as a specimen.
...
- [x] **Decide the structural guard against derivative-only synthesis.** — 2026-09-25. Beth: "don't care, cross off." Three candidate mechanisms, none chosen — this is Beth's call, not a fix to apply unilaterally:
...
- [x] **Wire `listening-game` into the `SessionStart` hook for real, or accept the non-invocation gap as recurring?** — 2026-09-25. Fixed, not accepted. `SessionStart` now injects `GAME-STATE.md`'s four gates line, current ledger tally (Claude/Beth), Microsoft Bob status, and hot-category names directly into session-open context — pipe-tested against the literal stored command under `sh` (matching this environment's actual hook runtime) before being wired in, per the `update-config` skill's verification procedure. Does not run the full `listening-game` skill (that still needs an explicit invocation for the deeper Step-1/1a-style scan) — this closes the narrower "the state isn't even visible at session start" half of the gap. Fourth report would have named the same gap a fourth time; this is the first fix, not another self-report patch.
- [x] **Atlassian MCP connector — real finding, not a hook fix.** — 2026-09-25. Beth: "I think we also need to fix the sessionstart to have Atlassian MCP? Instead of just Atlassian." Checked via `ListConnectors`: the org has two Atlassian connectors — "Atlassian - Adlumin" (`not_connected`) and "Atlassian MCP" (`installState: connected`, i.e. auth is fine at the org level) — but the real one shows `enabledInChat: false` for this session. That's a per-chat toggle in claude.ai connector settings, not a hook-fixable state: `SessionStart` hooks are shell commands (`command` type) and have no access to which connectors are toggled on for the current chat — confirmed by checking `claude mcp list`, which only sees `.mcp.json`-declared servers (`apex-agent`, `n-able`), not claude.ai-managed connectors like "Atlassian MCP" at all. **No hook can check this.** The existing `curl -I https://n-able.atlassian.net/` preflight line was never checking MCP auth — it only ever checked whether the website itself is reachable, and those are genuinely different failure modes (this session: website returns 403 Forbidden; the actual Atlassian MCP tools are separately unavailable per the connection-failure notice this session received, `Atlassian (404): CLIENT_HTTP_NOT_IMPLEMENTED`). Left the curl check as-is (it's honestly labeled "Atlassian:" not "Atlassian MCP:" and still gives real signal on the website itself) rather than building something that implies a check that doesn't exist. **Beth's action, not a fixable chore:** toggle "Atlassian MCP" on for this chat/session in claude.ai connector settings if Atlassian tool access is needed here.
...
- (Hand-carry the 2 PENDING mods — closed 2026-09-25, see that entry below.)
...
- [x] **Hand-carry the two PENDING required-mods, or wait on the graduation path?** — 2026-09-25. Beth: "don't remember what this is, cross off." MOD-001 (UX is problem-first) and MOD-002 (unknown is terminal) both landed in `CLAUDE.md` and both have paste-ready text in `briefs/required-mods-log.md`, awaiting propagation to `acp-core-main-3/specs/_ux/acp-ux.md` / the speckit constitution. **2026-08-21 — Beth: "dunno." Left open, not decided either way.** Also worth knowing now: `acp-core-main-3/specs/_ux/acp-ux.md` has an Aug 4 mtime — it's been edited in place within this project since the original download, so any hand-carry writes into Beth's own working copy, not a pristine upstream file. The mechanism is still the open chore below ("Design the resolved-divergence → real-spec-text graduation path"), parked since 2026-08-03.
...
- [ ] **Learnings review — turn the conduct-ledger error patterns into structural fixes, not resolutions to try harder.** *(Left open, 2026-09-25 — Beth: "leave — note to redesign next week." Not superseded by the four-gate redesign; take a fresh pass.)* Added because Microsoft Bob triggered 2026-08-27 (5th headmaster trip; 6th "Routed work back to Beth" Miss). The lift condition Beth set: *"Let me know when you figure out how to learn from your errors and are ready to be Claude again. But not before Monday (2026-08-31)."*
...
- [x] **Fix `listening-game` non-invocation structurally, or accept it as recurring?** Second known instance (2026-08-21, 2026-09-01) of the same session-start gap. — 2026-09-25. Fixed — see the 2026-09-25 entry above (`SessionStart` now injects game state directly).
- (Item 5, hand-carry the PENDING mods, closed 2026-09-25 — see that entry above.)

### Existing Open Items

- [ ] **ACP → Harness rename across prototypes and product-facing copy.** `[mechanical]` *(2026-09-25 — Beth: "do it.")*
...
- [x] **Get live GitHub access to `nable-fusion/acp-core`.** — 2026-09-25. Beth: "don't care, not happening, cross off."
...
- [ ] **Think through the Ten Types gaps surfaced 2026-08-12, plus Claude Projects for Harness/ACP.** *(2026-09-25 — Beth: "should probably do this with new Vertesia info, etc.")*
...
- [ ] **Re-run Idea Chess on the Package 3/Practice Companion/Aggregator/Visibility/Package 5 concepts through a kitting lens, not just automation. In progress this week of 2026-08-19 — not strictly gated to Friday chores-day. Sequenced AFTER the MVP-doc noodling below, not in parallel.** *(2026-09-25 — Beth: "do this.")*
...
- [x] **Think through abductive coding as a team sport — non-researchers coding rows, routed by domain.** — 2026-09-25. Beth: "don't care, not happening, cross off."
...
- [x] **Design the resolved-divergence → real-spec-text graduation path.** — 2026-09-25. Beth: "don't need — doing as part of another work stream, cross off." *Parked 2026-08-03 — needs more context and thought before building, per Beth.*
...
- [ ] **File the three orphaned secondary-research source files into the real datadump, not just `scout_input/`.** *(2026-09-25 — Beth: "fix.")*

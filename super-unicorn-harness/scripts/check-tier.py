...
Exit codes:
    0   tiering reported
    5   BROKEN — cannot evaluate: a gate file named in project.conf is missing, or
        zero acceptance criteria were parsable across all three gates. Both are
        false-green guards: reporting "0 open criteria, all clear" when nothing
        could be READ is the failure this code exists to prevent. 5 is the suite-wide
        "cannot evaluate" code — see EXIT-CODES.md; it is shared with check-trace.sh,
        check-design.py and ux-score.py on purpose, not by accident.
